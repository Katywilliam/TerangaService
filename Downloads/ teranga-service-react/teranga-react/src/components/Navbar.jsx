  ...
  return (
    <header className="bg-white sticky top-0 z-50 shadow-sm border-b border-gray-100">
      <nav className="max-w-7xl mx-auto px-6 flex items-center justify-between h-16">
        ...
        <div className="hidden md:flex items-center gap-1">
          {navLinks.map(link => (
            <Link
              key={link.to}
              to={link.to}
              className={`px-3 py-2 rounded-lg text-sm font-medium transition-all duration-150 no-underline ${
                location.pathname === link.to
                  ? 'bg-green-50 text-[#3A9E3A]'
                  : 'text-gray-600 hover:bg-gray-50 hover:text-[#3A9E3A]'
              }`}
            >
              {link.label}
            </Link>
          ...
          {isAuth ? (
            <div className="relative" onBlur={() => setDropdownOpen(false)}>
              ...
              >
                <span className="w-6 h-6 bg-[#3A9E3A] rounded-full flex items-center justify-center text-white text-xs font-bold">A</span>
                Mon compte
              ...
              {dropdownOpen && (
                <div className="absolute right-0 top-full mt-2 bg-white border border-gray-200 rounded-xl shadow-lg w-48 overflow-hidden z-50">
                  <Link to="/dashboard/client" className="flex items-center gap-2 px-4 py-3 text-sm text-[#1B3A6B] hover:bg-gray-50 no-underline border-b border-gray-100">
                    <Icon name="dashboard" size={14} /> Dashboard
                  </Link>
                  <button className="flex items-center gap-2 w-full px-4 py-3 text-sm text-red-500 hover:bg-red-50 text-left">
        ...
        <button
          className="md:hidden p-2 border border-[#1B3A6B] rounded-lg"
        ...
        <div className="md:hidden bg-[#142d55] border-t border-white/10">
          <div className="px-4 py-2">
            ...
            <div className="flex gap-2 pt-4 pb-2">
              <Link to="/connexion" onClick={() => setMobileOpen(false)} className="flex-1 text-center text-white text-sm py-2.5 border border-white/30 rounded-lg no-underline hover:bg-white/10 transition-all">
                Connexion
              </Link>
              <Link to="/inscription" onClick={() => setMobileOpen(false)} className="flex-1 text-center text-white text-sm py-2.5 bg-[#3A9E3A] rounded-lg no-underline font-semibold hover:bg-[#2d8a2d] transition-all">
                Inscription
              </Link>
            </div>
          </div>
